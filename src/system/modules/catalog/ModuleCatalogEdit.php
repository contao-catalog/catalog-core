...
class ModuleCatalogEdit extends ModuleCatalog
{
  ...
  protected function parseInputWidget($fieldName, array $fieldConfig, Widget $objWidget)
  {
    ...
    if ($objWidget instanceof FormFileUpload)
    {
      $result = $objWidget->parse() . $this->objWidgetUpload->parse();
    }

    elseif (in_array($fieldConfig['eval']['rgxp'],
        array('date', 'time', 'datim')))
    {
      ...
      // widget is passed to template
      $objWidget->datepicker = $this->datePicker($objWidget->value,
   ...
   */
  protected function parseFormTemplate(array $arrInputFields, array $arrWidgets,
      $blnAddGeneralError, $blnUseMultipart)
  {
